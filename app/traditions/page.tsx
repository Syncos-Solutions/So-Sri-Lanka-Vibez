...
import { traditionSubcategories } from "@/data/traditions"

export default function TraditionsPage() {
  const [scrollY, setScrollY] = useState(0)
  const [searchQuery, setSearchQuery] = useState("")
  const [filteredSubcategories, setFilteredSubcategories] = useState(traditionSubcategories)
  ...
  useEffect(() => {
    if (searchQuery.trim() === "") {
      setFilteredSubcategories(traditionSubcategories)
    } else {
      const filtered = traditionSubcategories.filter(
        (item) =>
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="relative h-96 overflow-hidden">
        <img
          src="/.jpg?height=400&width=1920&query=Sri%20Lankan%20traditions%20culture%20heritage%20customs"
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 to-black/40 flex flex-col justify-center items-start p-8 md:p-16">
          ...
          <h1 className="font-serif text-5xl md:text-6xl font-bold text-white mb-4">Sri Lankan Traditions</h1>
          <p className="text-white/90 text-lg max-w-2xl">
            Experience the rich cultural heritage and traditional practices of Sri Lanka
          </p>
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-center text-foreground/70 mb-8 scroll-fade-in">
            Explore different aspects of Sri Lankan culture
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredSubcategories.length > 0 ? (
              filteredSubcategories.map((subcategory, index) => (
                <Link
                  key={subcategory.id}
                  href={`/traditions/${subcategory.id}`}
              ...
              <div className="col-span-full text-center py-12">
                <p className="text-lg text-muted-foreground">No traditions found matching "{searchQuery}"</p>
              </div>
            )}
          </div>
        </div>
