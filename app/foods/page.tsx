...
import { foodSubcategories } from "@/data/foods"

export default function FoodsPage() {
  const [scrollY, setScrollY] = useState(0)
  const [searchQuery, setSearchQuery] = useState("")
  const [filteredSubcategories, setFilteredSubcategories] = useState(foodSubcategories)
  ...
  useEffect(() => {
    if (searchQuery.trim() === "") {
      setFilteredSubcategories(foodSubcategories)
    } else {
      const filtered = foodSubcategories.filter(
        (item) =>
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="relative h-96 overflow-hidden">
        <img
          src="/.jpg?height=400&width=1920&query=Sri%20Lankan%20food%20cuisine%20spices%20traditional%20cooking"
          alt="Sri Lankan Foods"
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 to-black/40 flex flex-col justify-center items-start p-8 md:p-16">
          ...
          <h1 className="font-serif text-5xl md:text-6xl font-bold text-white mb-4">Taste of Sri Lanka</h1>
          <p className="text-white/90 text-lg max-w-2xl">
            Discover the authentic flavors and culinary traditions of Sri Lanka
          </p>
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          <h2 className="font-serif text-4xl md:text-5xl font-bold text-center mb-4 scroll-fade-in">Food Categories</h2>
          <p className="text-center text-foreground/70 mb-8 scroll-fade-in">
            Explore different types of Sri Lankan cuisine
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredSubcategories.length > 0 ? (
              ...
              <div className="col-span-full text-center py-12">
                <p className="text-lg text-muted-foreground">No foods found matching "{searchQuery}"</p>
              </div>
            )}
          </div>
        </div>
