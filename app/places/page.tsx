...
import { placeSubcategories as importedPlaceSubcategories } from "@/data/places"

export default function PlacesPage() {
  const [scrollY, setScrollY] = useState(0)
  const [searchQuery, setSearchQuery] = useState("")
  const [filteredSubcategories, setFilteredSubcategories] = useState(importedPlaceSubcategories)
  ...
  useEffect(() => {
    if (searchQuery.trim() === "") {
      setFilteredSubcategories(importedPlaceSubcategories)
    } else {
      const filtered = importedPlaceSubcategories.filter(
        (item) =>
  ...
  return (
    <main className="min-h-screen bg-background">
      ...
      <section className="relative h-96 overflow-hidden">
        <img
          src="/.jpg?height=400&width=1920&query=Sri%20Lanka%20places%20scenic%20landscape%20mountains%20beaches"
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-black/70 to-black/40 flex flex-col justify-center items-start p-8 md:p-16">
          ...
          <h1 className="font-serif text-5xl md:text-6xl font-bold text-white mb-4">Explore Places</h1>
          <p className="text-white/90 text-lg max-w-2xl">Discover the most stunning destinations across Sri Lanka</p>
        </div>
      </section>
      ...
      <section className="py-20 px-4 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-center text-foreground/70 mb-8 scroll-fade-in">
            Choose a category to explore different types of places
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredSubcategories.length > 0 ? (
              ...
              <div className="col-span-full text-center py-12">
                <p className="text-lg text-muted-foreground">No places found matching "{searchQuery}"</p>
              </div>
            )}
          </div>
        </div>
